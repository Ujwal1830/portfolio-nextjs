...
const DATA = {
  ...
  contact: {
    social: {
      GitHub: {
        name: "GitHub",
        url: "https://github.com/Ujwal1830",
        icon: Icons.github,
      },
      LinkedIn: {
        name: "LinkedIn",
        url: "https://www.linkedin.com/in/ujwal-yangalwar-344961128/",
        icon: Icons.linkedin,
      },
      email: {
        name: "Send Email",
...
export function CustomDock({ orientation }: { orientation: "vertical" | "horizontal" }) {
  ...
  return (
    <div className={
        `${
            isMobile
            ? "fixed right-1/2 left-1/2 bottom-5 "
            : "fixed left-40 top-1/2 bottom-1/2 "} flex items-center justify-center`
        }>
        <TooltipProvider>
            { mounted && (
                <Dock direction="middle" orientation={
